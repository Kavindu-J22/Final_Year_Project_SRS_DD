...
const identityService = {
  ...
  getHistory:      (limit=100) => client(ML.identity).get(`/history?limit=${limit}`).then((r) => r.data),
...
const incidentService = {
  ...
  getRuleStore:    ()                        => client(ML.incident).get('/store'                    ).then((r) => r.data),
  importRules:     (rules)                   => client(ML.incident).post('/import-rules', { rules } ).then((r) => r.data),
  exportRules:     ()                        => client(ML.incident).get('/export-rules'             ).then((r) => r.data),
  deleteRule:      (ruleId)                  => client(ML.incident).delete(`/rules/${ruleId}`       ).then((r) => r.data),
...
const timelineService = {
  ...
  getMetrics:    ()       => client(ML.timeline).get('/metrics'                ).then((r) => r.data),
  searchEntity:  (entity, field='ip_address') =>
    client(ML.timeline).get(`/search/${encodeURIComponent(entity)}?field=${field}`).then((r) => r.data),
